...
glm::mat4 lightMSecond; //второй источник света
...
double coifficient = 1; //коэффициент для diffuse и specular
...
bool isMouseButtonPressed = false; //флаг нажатия на левую кнопку мыши
bool filledTriangles = true; //флаг на закрашивание треугольников
		...
		while (in)// если очередная строка считана успешно
		{
			...
			if ((str.find_first_not_of(" \t\r\n") != std::string::npos) && (str[0] != '#'))// прочитанная строка не пуста и не комментарий
			{
				...
				else if (cmd == "mesh")// набор точек
				{
					...
					while (N > 0)// пока не считали все треугольники
					{
...
void cursorPos_callback(GLFWwindow* window, double xpos, double ypos)// обработчик положения курсора
{
	...
	else if (isMouseButtonPressed) //относительно P
	{
		glm::vec3 n = glm::vec3(lastY - ypos, lastX - xpos, 0);
		glm::mat4 M = rotateP(glm::length(n) * 0.002f, n, P);
		/*glm::vec3 u_new = glm::mat3(M) * glm::vec3(0, 1, 0);*/
		glm::vec3 S_new = glm::vec3(M * glm::vec4(0, 0, 0, 1));// переход к СКН в которой начало координат в новой точке, а направление наблюдения - в точку P
...
{
	if (action == GLFW_PRESS && button == GLFW_MOUSE_BUTTON_LEFT) //если нажата левая кнопка мыши
	{
		isMouseButtonPressed = true;
	}
	else if (action != GLFW_PRESS) //если не нажата
	{
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	if (action != GLFW_RELEASE) { // если клавиша нажата
		...
		{
			initWorkPars();
			coifficient = 1; //возваращаем стандартное значение
			break;
		}
		case GLFW_KEY_3:
			pType = Perspective;
		...
		{
			if (mode == GLFW_MOD_SHIFT) {
				glm::mat4 M = rotateP(0.1, glm::vec3(1, 0, 0), glm::vec3(0, 0, -dist)); // матрица вращенияотносительно точки P
		...
		case GLFW_KEY_G: //разворот камеры на 0.1 радиан по часовой стрелке относительно, параллельной Ox
		{
			if (mode == GLFW_MOD_SHIFT) //относительно P
			{
			...
			else //относительно начала координат
			{
				glm::mat4 M = glm::rotate(-0.1f, glm::vec3(1, 0, 0)); // матрица вращения относительно Ox по часовой
				glm::vec3 u_new = glm::mat3(M) * glm::vec3(0, 1, 0); // вращение направления вверх
		...
		case GLFW_KEY_F: //разворот камеры на 0.1 радиан против часовой стрелке относительно оси, параллельной Oy
		{
			if (mode == GLFW_MOD_SHIFT)//относительно P
			{
			...
			{
				glm::mat4 M = glm::rotate(0.1f, glm::vec3(0, 1, 0)); //вращение против часовой относительно Oy
				glm::vec3 u_new = glm::mat3(M) * glm::vec3(0, 1, 0);//направление вбок
				glm::vec3 p_new = glm::vec3(M * glm::vec4(0, 0, -1, 1));//нормализация точки направления
				T = glm::lookAt(glm::vec3(0, 0, 0), p_new, u_new) * T;
			}
			break;
		}
		case GLFW_KEY_H: //разворот камеры на 0.1 радиан по часовой стрелке относительно оси, параллельной Oy
		{
			if (mode == GLFW_MOD_SHIFT)//относительно P
			{
				glm::mat4 M = rotateP(-0.1, glm::vec3(0, 1, 0), glm::vec3(0, 0, -dist));
...
int main()
{
	...
	// Создаем окно
	//int x = 500; //переменная для оперделения размера приложения на экране
	...
	glfwSetCursorPosCallback(window, cursorPosSave_callback);
	//назначение обработчика прокрутки колеса мыши
	glfwSetScrollCallback(window, scroll_callback);
	//обработчик событий срабатывает, когда нажимается или отжимается какаято кнопка мыши
